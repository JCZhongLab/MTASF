...
std::vector<PrsmStrPtrVec> PrsmCluster::groupProteins(const PrsmStrPtrVec &prsm_ptrs) {
  ...
  int max_shift_number = 0;   //b2_5ge_ms2.topmg_evalue 中最大的unexpected_ptm_num
...
void PrsmCluster::setClusterId(PrsmStrPtrVec& prsm_ptrs, double ppo) {
	//不同unexpected_ptm_num的prsm归为一组
  std::vector<PrsmStrPtrVec> proteo_groups = groupProteins(prsm_ptrs);

  // find zero ptm clusters
   //unexpected_ptm_num为0的一组prsm中seq_name、ProteoformStartPos、 ProteoformendPos相同而且adjusted_prec_mass之差小于ppo的归为一类clusters
  std::vector<PrsmStrPtrVec> clusters = getZeroPtmList(proteo_groups[0], ppo);


  //对unexpected_ptm_num为非0的所有prsm划分
  //满足prsm中seq_name、ProteoformStartPos、 ProteoformendPos相同而且adjusted_prec_mass之差小于ppo并且
  //shift数量一样、 MassShift满足特定要求的归为一类clusters
...
void PrsmCluster::process() {
  ...
  setProtId(prsm_ptrs);   //同名的proteins为同一个protid， e-value越小id越小
  setClusterId(prsm_ptrs, ppo_);  
  //根据SpectrumId和precursor_id排序，
