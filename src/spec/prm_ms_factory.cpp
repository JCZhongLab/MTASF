...
void addTwoMasses(PrmPeakPtrVec &list, int spec_id, DeconvPeakPtr deconv_peak_ptr,
  ...
  ///n_term_mass 当前峰值mass 减当前质谱activation的n_ion的shift  得到ORIGINAL  PrmPeak
  ...
  //prec_mono_mass 减当前质谱activation的c_ion的shift  得到REVERSED  PrmPeak
}
...
void addSuffixTwoMasses(PrmPeakPtrVec &list, int spec_id, DeconvPeakPtr deconv_peak_ptr,
  ...
  list.push_back(new_peak_ptr);
  ///// 当前峰值mass 减当前质谱activation的c_ion的shift再减水质量，c_res_mass   得到ORIGINAL  PrmPeak
  ...
  list.push_back(reverse_peak_ptr);
  /////prec_mono_mass 减当前质谱activation的n_ion的shift再减水质量  得到REVERSED  PrmPeak
}
...
PrmMsPtr geneMsTwoPtr(DeconvMsPtr deconv_ms_ptr, int spec_id, SpParaPtr sp_para_ptr,
  ...
  for (size_t i = 0; i < deconv_ms_ptr->size(); i++) {
    addTwoMasses(list, spec_id, deconv_ms_ptr->getPeakPtr(i), prec_mono_mass,   //spec_id即单个光谱中每个峰的序号
  ...
  //min_mass : 50.0 过滤规则大于min_mass 且小于prec_mono_mass - min_mass
