...
void DprProcessor::init() {
  ...
  ModPtrVec var_mod_ptr_vec = mod_util::readModTxt(var_mod_file_name)[2];  //返回的any对应的mod
  ...
  mass_table_ = mass_table_util::geneMassTable(mng_ptr_); //读取mass_table.txt生成一个map ，第一列作为key 后续作为value
